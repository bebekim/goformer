...
| 4 | (Addendum, direct follow-up) This spec's own result showed generations 5-8 plateauing/regressing (§11), diagnosed as exactly the "revisit if justified" condition #3 named. Implement the buffer now vs. leave it as a documented-but-untested future lever. | **Accepted: implemented, tested** | Condition #3 set for exactly this outcome was met. `run_generations.sh` gained `BUFFER=1` (new default) and `START_GEN` (resume support, needed because the follow-up run was interrupted once). Result: `docs/board-specification.md` §12 — the plateau/regression is gone (tight ~2.31-2.39 band through generation 10, vs. §11's collapse to 2.84-2.93), and the fixed lineage clearly beats its own starting point (77.8% win rate, +14.06 mean margin, gen10 vs. gen1) where §11's non-buffer lineage couldn't (46.2%, a coin flip). Filed as an addendum here rather than a new spec since it's a direct, planned test of this spec's own diagnosed next lever, not a new goal — see §12 for the honest caveats (the fixed run isn't clearly stronger than §11's lucky-early-stop result in a direct head-to-head, and the endgame-termination weakness persists unchanged). |
