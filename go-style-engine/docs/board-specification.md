...
### 8f. Real human kifu instead of self-play data (`Specs/009`)

> **Correction (§8g): this section's headline finding was wrong, and
> the error is now understood.** "The value head does not learn on
> real human data" was actually "the value head does not learn
> *within `--early-stopping-patience 3`*" — a budget tuned on
> self-play data in §8c that turns out not to transfer to real human
> data's much slower value-learning curve. Given a longer patience
> budget, the *identical* `absolute` config on this *identical* 9x9
> dataset breaks through (val_value_loss 1.0 → 0.63 and still falling
> at epoch 33, vs. patience=3's epoch-16 cutoff). The text below is
> kept as written — including its now-superseded "Recommendation" — as
> the historical record of what was concluded and why; §8g has the
> corrected picture, the 13x13 extension that surfaced the error, and
> what's still genuinely open.

Everything above (§7-§8e) trains on data generated by this project's
own self-play — even at its best, from a *randomly-initialized,
never-trained* net (§8d/§8e's own repeated caveat). `Specs/009`
removes that generator entirely: 80 real human 9x9 games (professional/
broadcast/pair-go players, via a new sibling repo, `go-gibo-ingestion`,
filtered to genuinely human categories -- 419 of the source archive's
517 games turned out to be `Minigo` AI self-play, not human kifu, see
that spec's Decision Log #2), 3,878 positions, one-hot policy targets
(no MCTS search over a human game, only the move actually played) and
true final-game-outcome value targets, converted via
`data/kifu_to_experience.py` (coordinate mapping hand-verified against
a real game's original SGF: 0 mismatches across all 80 moves).

Same protocol as §8's own comparisons: `--seed 0 --val-fraction 0.2
--early-stopping-patience 3 --epochs 40`, `board_size=9`, default
architecture otherwise.

| config | best epoch | val_policy_loss | val_value_loss |
|---|---|---|---|
| absolute | 13 | 3.1432 | 1.0013 |
| relative | 8 | 3.2208 | 1.0030 |
| both | 12 | 3.1344 | 1.0029 |
| gab (gen_size=64, dropout=0) | 2 | 3.3626 | 1.0206 |
| gab_absolute (gen_size=64, dropout=0) | 4 | 3.4566 | 1.0003 |
| gab_absolute, gen_size=4, dropout=0.1 | 6 | 3.2911 | 1.0049 |
| gab_absolute, gen_size=16, dropout=0.1 (§8's settled default) | 6 | 3.3921 | 0.9989 |
| gab_absolute, gen_size=64, dropout=0.1 | 4 | 3.4149 | 0.9997 |

**Headline finding: the value head does not learn on real human data,
in any configuration tried — and this is not overfitting.** Every
config's `val_value_loss` sits within noise of 1.0, the exact MSE a
constant-zero predictor gets against a ±1 balanced target (checked
directly: the dataset's value labels split 50.3%/49.7%, ruling out a
label-imbalance artifact). Checked directly, not assumed: *train*
`value_loss` is equally flat at ~1.00-1.01 for the full run of every
config inspected epoch-by-epoch (`absolute`, `both`,
`gab_absolute gen_size=16`) — the model never fits the value target
even on its own training positions, so this isn't the "fits train,
fails val" shape every other overfitting finding in §8 has; it's a
flat failure to learn *any* board→outcome signal at all. Policy loss,
by contrast, learns normally in every config (train policy loss drops
steadily, down to 2.7-2.9 by epoch 12-16) — this is a value-head-
specific failure, not a broken run.

This directly contradicts §7's `absolute`/`both` results on self-play
data, where the identical architecture learned the value target
cleanly (down to 0.05-0.11) from *fewer* positions (1,506 self-play
vs. 3,878 kifu) — data volume alone doesn't explain the gap.

**Leading hypothesis, untested:** self-play games from a random,
never-trained net (§7/§8's data source throughout) may make the value
target easier to fit than it looks — final outcomes in weak,
noise-driven play plausibly correlate with simpler, more locally
learnable cues (a stray capture, an unanswered invasion near the end)
that a small 4-layer/64-dim net can pick up from a couple thousand
positions. Real human 9x9 games between strong players are close,
contested games where the outcome genuinely depends on accumulated
strategic judgment across the whole board — a harder function to
learn, and one this architecture/data scale may simply not be
sufficient for. This is a hypothesis, not a finding: distinguishing
"harder target" from "not enough human data" (80 games is a fraction
of even §7's already-small 30-game self-play set) would need either
much more human data or a deliberately matched-difficulty synthetic
comparison, neither attempted here.

**Scope caveats, stated as plainly as §7/§8's own:** one seed, one
board size, one small architecture size, 80 games (small even by this
doc's own standards) — a genuinely noisy regime, but the flatness of
the result (every single config within 0.03 of the 1.0 baseline, both
train and val) is not the kind of pattern that reads as sampling
noise; a real effect, even if the small sample means its *cause*
remains unpinned.

**What this means for the rest of §8's findings.** GAB's
content-dependence advantage (§8 Finding 1: GAB beats `relative` on
the value objective) doesn't show up here because *nothing* beats
baseline on the value objective here — this result doesn't overturn
§7/§8's self-play-data findings, it says those findings may be
specific to the self-play-data regime they were measured in, and
don't yet transfer to real human data at this scale. Policy loss *is*
comparable across configs here (3.13-3.46, `absolute`/`both` best,
`gab`/`gab_absolute` variants slightly behind) — closer to §7/§8's
pattern, though on too small a sample to call a clean win for either
family.

**Recommendation:** don't retune hyperparameters chasing this number
without more human data first — every other lever this doc has tried
(dropout, weight_decay, gab_gen_size) targets *capacity*, and this
result's flat-on-train shape says the problem isn't excess capacity
relative to data (that produces the overfitting shape §8 saw
elsewhere), it's insufficient learnable signal at this data volume for
this specific, harder target. More real human games
(`go-gibo-ingestion` only fetches the CWI 9x9 archive today; a larger
or supplementary kifu source would be the direct next lever) is the
honest next experiment, not another architecture sweep on the same 80
games.

### 8g. Correcting §8f: an early-stopping-patience artifact, not a learning failure

Extending `Specs/009` to 13x13 (this project's actual target board
size, unlike 9x9's cheap-comparability role throughout §7/§8) surfaced
the error in §8f's headline finding. `go-gibo-ingestion` was extended
(board size made a data field rather than a 9x9-only filter, plus
exact-duplicate detection) to pull 13x13 games out of the full CWI
corpus, which turns out to have 110 unique real professional 13x13
games (`EV[Pro 13x13 tournament]`, 102 distinct named players
including Cho Chikun, O Meien, Cho U, Takemiya Masaki; no AI/engine
markers found, unlike 9x9's `Minigo` contamination) — 11,312
positions via the same `kifu_to_experience.py` path as §8f, coordinate
mapping hand-verified the same way (0 mismatches across a real game's
80 moves).

**First pass, same protocol as §8f** (`--seed 0 --val-fraction 0.2
--early-stopping-patience 3 --epochs 40`, `board_size=13`):

| config | best epoch | val_policy_loss | val_value_loss |
|---|---|---|---|
| absolute | 10 | 3.4015 | 1.0021 |
| relative | 8 | 3.5884 | 1.0005 |
| **both** | **17** | **3.2893** | **0.1823** |
| gab (gen_size=64, dropout=0) | 2 | 3.8299 | 1.0015 |
| gab_absolute (gen_size=64, dropout=0) | 3 | 3.8513 | 1.0012 |
| gab_absolute, gen_size=4, dropout=0.1 | 4 | 3.5903 | 1.0001 |
| gab_absolute, gen_size=16, dropout=0.1 (§8's settled default) | 3 | 3.7231 | 1.0045 |
| gab_absolute, gen_size=64, dropout=0.1 | 3 | 3.8372 | 1.0004 |

Looked at first glance like a partial replication of §8f (still flat
at ~1.0 almost everywhere) — except `both` clearly broke through,
matching self-play's own §7 result (`both` learned cleanly there too).
That one exception is what didn't fit, and chasing *why* is what
overturned §8f.

**The mechanism.** `train.py`'s early stopping tracks *combined*
val loss (policy + value). Pulling each config's full epoch trace:
`absolute`'s `val_policy_loss` plateaus by epoch 6-10 (3.40-3.44,
essentially flat) while its value loss is *also* flat — so combined
loss stops improving quickly, and `--early-stopping-patience 3`
correctly fires at epoch 13, well before any value breakthrough could
happen even if one was coming. `gab_absolute`'s policy loss actively
*worsens* from epoch 3 (3.72 → 4.91 by epoch 6), triggering an even
earlier stop. `both`, uniquely, kept its `val_policy_loss` improving
all the way to epoch 9 (4.42 → 3.43) — so combined loss kept
improving through the value plateau on policy's strength alone,
buying `both` the extra epochs patience=3 denied everyone else. Its
value loss was flat through epoch 9 too, then broke free starting
epoch 10 (1.0 → 0.93 → 0.83 → 0.62 → ... → 0.18 by epoch 17). This is
a *training-budget* difference between configs, not necessarily an
architectural one.

**Testing that directly: give the "starved" configs the same runway.**
`absolute` re-run with `--epochs 60 --early-stopping-patience 15`:

| epoch | val_policy_loss | val_value_loss |
|---|---|---|
| 13 | 3.4413 | 0.9933 |
| 14 | 3.4518 | 0.9596 |
| 15 | 3.4915 | 0.6732 |
...
| 20 | 3.4367 | 0.1859 |
| 30 | 4.1762 | 0.1247 |
| 36 (stopped) | 4.8933 | 0.1129 |

**It breaks through too** — flat through epoch 13 (exactly where
patience=3 had cut it off), then the same kind of sharp drop `both`
showed, reaching value loss comparable to `both`'s best. (Policy loss
gets *worse* over this same stretch, 3.44 → 4.89 — a real
policy/value tradeoff during this late phase, not a free win; combined
loss still improves because value's drop dominates.)

**Re-testing §8f's original 9x9 claim the same way** — this is the
one that matters most, since §8f's "flat failure... a real effect"
language was written as a confirmed conclusion, not a hypothesis.
`absolute` on the *identical* 9x9 `Specs/009` dataset, `--epochs 80
...
| 28 | 3.6987 | 0.8827 |
| 30 | 3.8174 | 0.7971 |
| 33 (stopped) | 4.3344 | 0.6338 |

**Also breaks through**, starting almost exactly where §8f's
`--early-stopping-patience 3` run had already stopped (epoch 16) —
still falling at epoch 33 when this longer patience budget finally
exhausted, not yet converged the way the 13x13 runs got to. §8f's
9x9 dataset is smaller (3,878 vs. 13x13's 11,312 positions), which
plausibly explains the slower, still-incomplete breakthrough, but that
is itself untested here.

**One config does NOT replicate this pattern: `gab_absolute` at §8c's
settled tuning (`dropout=0.1, gab_gen_size=16`).** Same long-patience
treatment (`--epochs 60 --early-stopping-patience 15`) on 13x13:

| epoch | val_policy_loss | val_value_loss |
|---|---|---|
| 3 (best) | 3.7231 | 1.0045 |
| 6 | 4.9132 | 1.0027 |
| 10 | 9.1353 | 0.9998 |
| 14 | 12.0631 | 1.0002 |
| 18 (stopped) | 10.6958 | 1.0000 |

Value loss never moves off ~1.00 — but this isn't the same "flat,
stable, waiting for a breakthrough" shape `absolute`/`both` showed
before theirs. Policy loss *catastrophically diverges* (3.72 → 12+),
and early stopping (patience=15 from best epoch 3) fires at epoch 18,
before reaching anywhere near the epoch-14-ish region where the other
two configs' breakthroughs began. Whether `gab_absolute` would
*eventually* break through given even more patience, or whether it
genuinely can't while its policy is diverging this badly, is unknown
— untested past epoch 18. Don't retune GAB's hyperparameters to chase
this without more evidence; that repeats exactly the "tuning without
data" mistake §8a-§8b's early sections deliberately avoided.

**Corrected conclusion.** Real human-game value learning is not
fundamentally harder or broken, the way §8f concluded — every
config tested except `gab_absolute` at its current tuning eventually
learns it, at both board sizes, given enough epochs. The actual
finding is narrower and more useful: **`--early-stopping-patience 3`
(settled on self-play data in §8c, where value loss converges by
epoch 5-13) is calibrated to the wrong regime for real human game
data**, whose value head appears to plateau for 10-25+ epochs before
breaking through, not the 3-epoch window self-play needed. §8f's
"more human data" recommendation is superseded — the direct next
lever for any future kifu-based training run is a much longer
`--early-stopping-patience` (10-20, not 3) and correspondingly larger
`--epochs` budget, not more data collection. What's still genuinely
open, stated plainly rather than guessed at: *why* the plateau is so
much longer for real data than self-play data in the first place
(untested — could be §8f's original "harder target" hypothesis after
all, just manifesting as a longer warm-up rather than a hard ceiling),
and whether `gab_absolute` shares this pattern or has a genuinely
different, unresolved problem on real data.
...
`009`/`010` are now `done` -- §8f/§8g's real-human-kifu work is the
first item in this list that's actually been run. §8g corrected §8f's
own initial conclusion and found the real next lever isn't more data,
it's `--early-stopping-patience` tuned for kifu training specifically
(10-20, not self-play's 3) -- not yet its own numbered spec, and
`gab_absolute`'s unresolved divergence-vs-breakthrough question from
§8g is a second open item alongside it.
...
## 13. Making GAB actually viable: gradient clipping + self-play data

Every playing-strength checkpoint through §12 uses `pos_mode='both'`,
specifically *because* `gab_absolute` had an unresolved catastrophic
policy-divergence problem on real data (§8g). But GAB — Geometric
Attention Bias — is this project's actual named architecture
(Chessformer/Maia3-inspired, the reason this whole `docs/` line of
work exists). Using `'both'` as a permanent stand-in was never the
plan; this section is the diagnostic thread that made GAB usable, and
the multi-generation loop that tested whether the fix actually holds.

### 13a. Diagnosing the divergence

`train.py` had no gradient clipping anywhere in its training loop.
§8g's `gab_absolute` failure (`val_policy_loss` exploding 3.72 → 12+
within a handful of epochs on real one-hot kifu targets) is the
textbook signature. Four diagnostic runs, all on the exact 9x9 kifu
dataset §8g used, isolated the real cause:

| config | val_policy_loss (best → worst before stopping) | val_value_loss | shape |
|---|---|---|---|
| `gab_absolute`, gen_size=16, no clip (§8g, unchanged) | 3.72 → 12.06 | flat ~1.0 | wild divergence |
| + `--grad-clip 1.0` (new flag) | 3.35 → 6.23 | flat ~1.0 | ordinary overfitting |
| + `--grad-clip 1.0`, gen_size=4 | 3.35 → 4.38 | flat ~1.0 | milder overfitting |
| + `--grad-clip 1.0`, gen_size=4, `--weight-decay 0.01` | 3.36 → 4.40 | flat ~1.0 | no measurable change |

**`--grad-clip` (new `train.py` flag, default 0.0 — disabled, unchanged
behavior for every prior config)** wraps `loss.backward()` with
`torch.nn.utils.clip_grad_norm_`. It converts wild instability into
ordinary, well-understood overfitting — a real fix, not a workaround,
and cheap (one line, no architecture change). `weight_decay=0.01`
changed essentially nothing (3.3627/3.3608 mid-run, indistinguishable
from the no-decay run), reconfirming §8a's "weight_decay is inert"
finding in this new, real-data regime — not just on self-play data.

Grad-clipping alone still leaves ordinary overfitting on kifu data,
though, and value loss never breaks free of ~1.0 in any of these four
runs — early stopping (patience=15) keeps firing around epoch 20-21,
before value would need the same ~25+ epoch runway §8g found `absolute`
needs on this same tiny dataset. The real fix turned out to be a
different question entirely:

**Does GAB's instability come from real/small data, or from one-hot
targets specifically?** §7/§8's original GAB validation never showed
this problem — but it only ever trained on self-play data, which has
*soft* MCTS-visit-distribution policy targets, not kifu's one-hot
"this exact move was played" targets. Testing this directly: `gab_absolute`
+ `--grad-clip 1.0`, trained on real self-play data (16,628 positions,
the `'both'`-buffer loop's own accumulated shards from §12) instead of
one-hot kifu data:

```
epoch 1:  val_policy=2.7393  val_value=0.3735
epoch 5:  val_policy=2.4717  val_value=0.1656
epoch 11: val_policy=2.4515  val_value=0.0464  (best epoch)
epoch 26: val_policy=2.4957  val_value=0.0412  (early stopping)
```

Clean, stable convergence — no overfitting blowup, `val_policy_loss`
settling in a tight band, `val_value_loss` dropping smoothly to 0.03.
Best combined val loss 2.50, better than any config's result on real
kifu data (`'both'`'s own best there was ~4.14). **This narrows the
cause precisely: GAB's instability is specific to one-hot targets, not
real or small data in general** — `absolute`/`both` handle both
regimes fine; GAB apparently needs the softer, searched signal
self-play provides. This matches production Maia3's own design point
exactly: GAB ships alone, but trained on 169M *real games'* worth of
signal, not raw one-hot supervised moves.

### 13b. Does the fix hold across a real multi-generation loop?

One good training run doesn't prove the fix survives the dynamic
regime this project actually needs: a self-play refinement loop, where
each generation's training data is generated by the *previous*
generation's own weights, and the net is repeatedly warm-started
rather than trained fresh. `run_generations.sh` gained `--grad-clip`
passthrough (`GRAD_CLIP` env var, default 0.0, unchanged) and
`GAB_INTERMEDIATE_DIM` as an *independent* variable from `GAB_GEN_SIZE`
(a real bug, caught immediately: the first attempt to seed this loop
failed with a state_dict shape mismatch, because §13a's diagnostic
checkpoints were built with `--gab-gen-size 16` alone, leaving
`--gab-intermediate-dim` at its default 64 — `run_generations.sh` had
always tied the two together).

Seeded from §13a's clean self-play-trained checkpoint (and that
checkpoint's own 300-game starting buffer, copied over as GAB's
initial data), 10 generations, `pos_mode=gab_absolute`, `gen_size=16`,
`intermediate_dim=64`, `--grad-clip 1.0`, `PATIENCE=15`, otherwise
matching §12's setup exactly:

| generation | val_policy_loss | val_value_loss | best_epoch |
|---|---|---|---|
| 1 | 2.4007 | 0.0454 | 2 |
| 2 | 2.3715 | 0.0440 | 3 |
| 3 | 2.3959 | 0.0568 | 1 |
...
**No divergence, anywhere in 10 generations — including generation 5,
the exact point where §11's original (non-buffer) `'both'` lineage
collapsed to ~2.84-2.93.** GAB stayed in a tight 2.37-2.50 band the
whole way, a mild upward drift by the end (2.40→2.50) but nothing
resembling instability. This run was interrupted twice externally
(killed, not a bug) and resumed cleanly both times via `START_GEN` —
the result held together across those seams too.

One pattern worth flagging plainly, not glossing over: **`best_epoch`
is consistently very low — 1, 2, or 3, every single generation.**
`'both'`'s own loop had a much more varied pattern (`best_epoch` from
2 up to 41). This could mean GAB converges faster (a real advantage —
less compute per generation), or that it's finding a shallow optimum
quickly without necessarily using the data as thoroughly as `'both'`
does. Both interpretations are consistent with the numbers so far;
distinguishing them needs more than this one run.

### 13c. Does it actually play better? (real tournament, not just loss)

§12 already established that loss curves and head-to-head playing
strength don't perfectly track each other — so this section's loss
stability alone doesn't answer "is GAB viable," only "does it train
without breaking." The evaluation tooling itself needed a real fix
first: the tournament script (built in §11/§12) hardcoded
`pos_mode='both'` for *both* sides being compared, so it silently
failed with a shape-mismatch error the moment a GAB checkpoint was
passed in. Fixed to take independent `pos_mode`/`gab_gen_size`/
`gab_intermediate_dim` per side — verified working via a smoke test
before trusting any result from it.

| matchup | games | win rate (decided) | mean margin | undecided |
|---|---|---|---|---|
| GAB gen10 vs. `'both'` buffer-gen10 (§12) | 20 | 66.7% (8-4) | +1.50 | 8/20 (40%) |
| GAB gen10 vs. random-init (matched architecture) | 16 | **83.3%** (10-2) | **+12.08** | 4/16 |

GAB gen10 beats random-init decisively — a clearer margin than either
`'both'` lineage managed against random (§11: 73.3%/+8.2; §12:
69.2%/+9.65). Against `'both'`'s own best checkpoint, GAB has a real
edge in win rate (66.7%) but a much thinner one in score margin
(+1.50) — consistent with §12's own lesson that these two checkpoints,
despite different training histories, land closer together in
absolute strength than the loss numbers alone would suggest. The 40%
undecided rate in the GAB-vs-`'both'` matchup is notably higher than
most prior matchups this session, worth naming rather than averaging
away.

**Qualitative.** One inspected GAB gen10 self-play game is a genuinely
positive, distinct data point: it terminated *naturally* via a real
double-pass at move 129 — not the 162-move cap every previously
inspected game (across both `'both'` loops) ran into. Black won by
19.5 after a clean, sensible-looking game (varied opening/midgame
positions, no degenerate immediate-passing). But this is one example,
not a fixed weakness: the tournament data above shows 25-40% of games
still hit the move cap, comparable to or worse than `'both'`'s own
rates. Endgame termination remains a real, unresolved, architecture-
independent issue — not something GAB incidentally solved.

### 13d. Verdict

**GAB is now viable as `trans-go-former`'s production architecture.**
The catastrophic divergence that ruled it out in §8g had a real,
identified, fixed cause (gradient clipping plus training on searched/
soft-target data instead of one-hot kifu data) — not a papered-over
workaround. Ten full generations of self-play refinement, including
two external interruptions, showed no instability, and the resulting
checkpoint decisively beats a random-init net (83.3%, the best margin
any architecture achieved this session) and holds at least a modest
edge over the `'both'` lineage this session had been using as a
pragmatic stand-in.

What's not yet resolved, stated plainly: the low-`best_epoch` pattern
(§13b) is new and unexplained — worth watching as more generations
accumulate, not yet concerning. Endgame termination (§13c) is
unaffected by any of this session's fixes, on any architecture tried.
And this is one lineage, one seed — this doc's own standing
epistemic caution (§7/§8's "one seed, no sweep") applies here exactly
as much as everywhere else in it.
