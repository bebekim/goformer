# Ported from maxpumperla/deep_learning_and_the_game_of_go (MIT license),
# dlgo/goboard_fast.py. Board-size agnostic; used here at 13x13.
...
class MoveAge:
    """Tracks how long each stone has sat on the board (used by
    encoders that want move-recency features)."""
...
class GoString:
    """Stones linked by a chain of connected stones of the same color."""
...
class Move:
    """Exactly one of is_play, is_pass, is_resign is set."""
    ...
    def __eq__(self, other):
        return (
            self.is_play, self.is_pass, self.is_resign, self.point) == (
