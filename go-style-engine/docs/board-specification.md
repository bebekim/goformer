...
`009`/`010` are now `done` -- §8f's real-human-kifu result is the
first item in this list that's actually been run, and it opened a new
one: more real human game data (beyond the 80-game default this doc
used) as the next concrete lever on §8f's value-learning failure, not
yet its own numbered spec.
